...
/* В полосе слева пока только «Заметки»: остальные виды ждут переработки и до неё из меню
   убраны. Сами виды живы — открываются из палитры (Ctrl+K). Горячие клавиши 1-9 на
   переключение видов убраны отдельно (см. main.js) — упоминание о них здесь было хвостом.
...
function wireAside(it){
  ...
  /* ПРАВКА ТОЛЬКО ТЕКСТА — облегчённый путь для заголовка/описания. build() тут не нужен: он
     красит всё дерево заново и пересчитывает точки крепления КАЖДОЙ ноды к пустышкам, хотя от
     текста ни цвет, ни форма, ни связи не зависят. Заголовок отображается подписью узла — её
     patchLabel меняет точечно; тело узла на холсте не показывается вовсе, графу трогать нечего. */
  const обновитьТекст = (заголовок, панель=true)=>{
    touch(it); persist();
    renderNav();
    if(graph){ if(заголовок) graph._patchLabel(it.id, it); }
    else if(view!=="notes") render();
    if(панель) renderAside();
  };
  ...
  if(загл){
    загл.oninput = ()=>позже(()=>{ it.title = загл.textContent.trim(); обновитьТекст(true, false); });
  ...
  if(тело){
    ...
    тело.oninput = ()=>{ asideAutoGrow(тело); позже(()=>{ it.body = тело.value; обновитьТекст(false, false); }); };

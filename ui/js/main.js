...
function wireGlobal(){
  ...
  document.addEventListener("keydown",e=>{
    ...
    /* Ctrl+Shift+F — счётчик кадров в паутине. Стоит ВЫШЕ Ctrl+F: тот ловит клавишу F без
       оглядки на Shift и забрал бы сочетание себе. Счётчик нужен, чтобы судить о плавности по
       СВОЕМУ файлу и своей руке — любой замерочный стенд мягче живой работы (в нём нет ни
       наведения, ни правой панели, ни настоящих событий мыши). Показывает не только кадры в
       секунду, но и из чего состоит кадр: физика, свечение, прочее. Настройка живёт в
       S.settings, потому что Graph пересоздаётся на каждый render() — на экземпляре счётчик
       гас бы от любой правки данных. */
    if((e.ctrlKey||e.metaKey) && e.shiftKey && e.code==="KeyF"){
      e.preventDefault();
      S.settings.graphFps=!S.settings.graphFps;
      if(graph){ graph._wake(); graph._tick(true); }
      toast(S.settings.graphFps?"Счётчик кадров включён":"Счётчик кадров выключен",{icon:"ti-activity"});
      return;
    }
    ...
    if((e.ctrlKey||e.metaKey) && e.code==="KeyZ"){
      e.preventDefault();
      /* Автоповтор ОС игнорируем: держат клавишу — Windows шлёт keydown 20-30 раз в секунду,
         а каждый шаг отката раньше синхронно писал на диск ВЕСЬ файл (_undoApply в core.js).
         Секунда зажатой клавиши превращалась в три десятка полных записей подряд — заметный
         фриз прямо во время обычной работы с графом. Один физический нажим — один шаг;
         быстро откатить несколько действий подряд — жать клавишу несколько раз, а не держать. */
      if(e.repeat) return;
    ...
    if((e.ctrlKey||e.metaKey) && view==="notes" && graph){
      ...
      if(e.code==="KeyG" && e.shiftKey){ e.preventDefault(); graph.heatNext(); return; }
      if(e.code==="KeyG"){ e.preventDefault(); graph.toggleHeat(); return; }
    }
    ...
    /* Цифр 1–9 для переключения видов БОЛЬШЕ НЕТ (убрано по просьбе КРОЛИКА). Одиночная клавиша
       без модификатора слишком легко срабатывает мимо поля ввода — ровно та же причина, по
       которой раньше убрали «N» для новой ноды. Виды переключаются полосой слева и Ctrl+K. */
  });
...
async function boot(){
  ...
  if(жилойФайл(loaded)){ S=sanitizeState(Object.assign(defaultState(),loaded)); }
    ...
    seedDemo();
    /* НОВАЯ УСТАНОВКА ТУР НЕ ВИДИТ. Он рассказывает, ЧТО ИЗМЕНИЛОСЬ, а человеку, который
       открыл приложение впервые, менять было нечего: он всех этих кнопок и не знал. Помечаем
       тур просмотренным ДО первой записи, чтобы отметка сразу уехала на диск вместе с демо. */
  ...
  // «Что нового» после обновления — один раз на версию (см. ТУР выше)
  turMaybeShow();
...
/* ===========================================================
   ТУР «ЧТО НОВОГО» — один раз на версию, после обновления.

   Зачем не заметки к релизу: их читают на GitHub, а обновление прилетает кнопкой прямо в
   приложении, и человек оказывается в новой версии, не открыв ни строчки описания. Новая
   кнопка при этом ничем себя не выдаёт — про домик «Держать раскладку» без подсказки узнать
   неоткуда, пока не ткнёшь наугад.

   Кому показываем: ТОЛЬКО тому, кто обновился. Новая установка засевается демо и сразу
   помечается видевшей тур (см. boot) — новичку нечего рассказывать про «что изменилось», он
   всего этого и не видел. Побочно это же спасает сценарии: демо-данные тур не открывают, и
   он не перекрывает интерфейс посреди проверок.

   Шаг с ЦЕЛЬЮ подсвечивает живой узел интерфейса и показывает на него хвостиком выноски.
...
/* Релиз накопил пять самостоятельных вещей (статусы, срок+приоритет, «что горит»,
   ...
   на глаз, а взяты теми же именами CSS-переменных, что рисует сам граф (graph.js: _palette,
...
const ТУР=[
  {v:"2.0.5", значок:"ti-circle-dot", ttl:"Семь статусов — видно без открытия ноды", виз:ТУР_ВИЗ_СТАТУСЫ,
   ...
   before(){ view="notes"; notesMode="graph"; render(); }},
  {v:"2.0.5", значок:"ti-eye", ttl:"Срок и приоритет — видно сразу", виз:ТУР_ВИЗ_СРОК,
   ...
   before(){ view="notes"; notesMode="graph"; render(); }},
  {v:"2.0.5", значок:"ti-flame", ttl:"Что горит — режим по одной кнопке", цель:"#g-heat", виз:ТУР_ВИЗ_ЖАР,
   ...
   before(){ view="notes"; notesMode="graph"; render(); }},
  {v:"2.0.5", значок:"ti-bell", ttl:"Напоминание — на любой ноде", цель:".event-ctl [data-eventpick]",
  ...
  {v:"2.0.5", значок:"ti-bell-ringing", ttl:"Уведомление даже при свёрнутом окне", виз:ТУР_ВИЗ_ТОСТ,
  ...
  {v:"2.0.5.1", значок:"ti-drag-drop", ttl:"Тащи всё выделенное сразу",
   txt:"Выдели ноды рамкой или с Shift и тяни любую из них — остальные поедут следом.",
   before(){ view="notes"; notesMode="graph"; render(); }},
  {v:"2.0.5.2", значок:"ti-copy", ttl:"Ctrl+тащи — копия под рукой",
   txt:"Зажми Ctrl и потяни выделенное: под рукой окажутся копии со всеми связями, а оригиналы останутся на месте.",
   before(){ view="notes"; notesMode="graph"; render(); }},
];
/* СЛОЁНЫЙ ТУР: человек видит только то, чего ещё не видел. Каждый шаг помечен версией, в которой
   появился, и при открытии список фильтруется по `tourSeen`: обновившийся с 2.0.5 получит один
   шаг про перетаскивание, пришедший с 2.0.4 — всё подряд. Раньше тур был плоским, и заплатка
   заставляла человека заново листать то, что он уже читал.
   Версии сравниваем ЧИСЛАМИ по частям, а не строками: строкой "2.0.10" МЕНЬШЕ "2.0.9", и на
   десятом патче тур молча перестал бы показываться — поломка, которую заметили бы через полгода.
   ГЛУБИНУ держать в два релиза: шаги позапрошлой версии убирать при выпуске новой, иначе
   обновившийся через полгода получит простыню, а сборка потащит все старые клипы. */
function turVerNewer(a, b){
  if(!b) return true;                                  // не видел ничего — значит ново всё
...
function turNewSteps(){ return ТУР.filter(ш => turVerNewer(ш.v||ТУР_ID, S.settings.tourSeen)); }
/* Какой список листаем прямо сейчас. Из меню «Что нового» — ВЕСЬ тур: туда человек лезет сам,
   и обрезать ему нечего. После обновления — только новые шаги; если их вдруг нет (открыли руками
   при совпавшей отметке), показываем всё, чтобы кнопка не открывала пустоту. */
let _турПоказ=ТУР, _турНовые=null;
/* СПИСОК НОВОГО ЗАМОРАЖИВАЕТСЯ НА ПОКАЗЕ ПЛАШКИ, а не считается при нажатии кнопки. Плашка
   ставит `tourSeen` сразу, как только человек её увидел (иначе пропущенная плашка вернётся
   при следующем запуске), — значит к моменту клика по «Что нового» новых шагов уже НОЛЬ, и
   тур показывал бы всё подряд. Ровно это и вылезло при первой проверке: обновившийся с 2.0.5
   получал все шесть подсказок вместо одной. */
function turFreezeNew(){ _турНовые=turNewSteps(); return _турНовые; }
function turOpen(все){
  const нов = (_турНовые && _турНовые.length) ? _турНовые : turNewSteps();
  _турПоказ = (все || !нов.length) ? ТУР : нов;
  turStep(0);
}
let _турШаг=0, _турУзел=null;
function turDone(){
  if(_турУзел){ _турУзел.remove(); _турУзел=null; }
  document.querySelectorAll(".tour-lit").forEach(n=>n.classList.remove("tour-lit"));
  document.removeEventListener("keydown", turEsc, true);
  window.removeEventListener("resize", turPlace);
  if(S.settings.tourSeen!==ТУР_ID){ S.settings.tourSeen=ТУР_ID; persist(); }
}
function turEsc(e){ if(e.key==="Escape"){ e.preventDefault(); e.stopPropagation(); turDone(); } }
/* Позицию считаем ОТДЕЛЬНОЙ функцией и зовём её ещё на resize: окно у Мыслика меняет размер
   мышью за любой край, и выноска, посчитанная один раз, отъезжала бы от своей кнопки. */
function turPlace(){
  if(!_турУзел) return;
  const шаг=_турПоказ[_турШаг], цель=шаг.цель ? document.querySelector(шаг.цель) : null;
  const хвост=$(".tour-tail",_турУзел);
  const w0=_турУзел.offsetWidth||320, h0=_турУзел.offsetHeight||140;
  if(!цель){
    _турУзел.classList.add("mid"); _турУзел.classList.remove("up");
    // не даём уехать за верх/левый край: подсказка со сравнением высокая, и на низком окне
    // центрирование «в минус» срезало бы ей шапку вместе с крестиком
    _турУзел.style.left=Math.max(8, Math.round((window.innerWidth-w0)/2))+"px";
    _турУзел.style.top=Math.max(8, Math.round((window.innerHeight-h0)/2))+"px";
    if(хвост) хвост.style.display="none";
    return;
  }
  _турУзел.classList.remove("mid");
  if(хвост) хвост.style.display="";
  const r=цель.getBoundingClientRect(), w=w0, h=h0;
  // под целью, если снизу есть место; иначе над ней — выноска не должна уезжать за окно
  const снизу = r.bottom+14+h <= window.innerHeight-8;
  let left=Math.round(r.left+r.width/2-w/2);
  left=Math.max(10, Math.min(left, window.innerWidth-w-10));
  const top=снизу ? Math.round(r.bottom+14) : Math.round(r.top-h-14);
  _турУзел.style.left=left+"px"; _турУзел.style.top=top+"px";
  _турУзел.classList.toggle("up", !снизу);
  if(хвост) хвост.style.left=Math.round(r.left+r.width/2-left)+"px";
}
function turStep(n){
  _турШаг=n;
  const шаг=_турПоказ[n]; if(!шаг){ turDone(); return; }
  if(шаг.before) try{ шаг.before(); }catch(e){ console.warn("[тур] шаг не подготовился:", e); }
  document.querySelectorAll(".tour-lit").forEach(k=>k.classList.remove("tour-lit"));
  if(!_турУзел){
    _турУзел=el("div","tour-pop"); _турУзел.id="tour-pop";
    document.body.appendChild(_турУзел);
    document.addEventListener("keydown", turEsc, true);
    window.addEventListener("resize", turPlace);
  }
  const последний = n===_турПоказ.length-1;
  /* Шаг со сравнением ШИРЕ обычного: в 340 px два окошка ужимались до почтовых марок, и
  ...
  _турУзел.innerHTML=`<span class="tour-tail"></span>
    <div class="tour-pop-inner">
      ...
      <div class="tour-foot">
        <span class="tour-dots">${_турПоказ.map((_,i)=>`<i class="${i===n?"on":""}"></i>`).join("")}</span>
  ...
  /* «Включить сейчас» — тур не только рассказывает, но и делает. Жмём ТУ ЖЕ кнопку, а не зовём
     toggleHome напрямую: у неё на клике висит и переключение вида кнопки, и тост, и пробуждение
     кадров — обойти её значит однажды разойтись с ней поведением. Повторное нажатие не мешает:
     если дом уже включён, не трогаем вовсе, иначе кнопка бы его ВЫКЛЮЧИЛА. */
  const дейст=$("#tour-do",_турУзел);
  if(дейст) дейст.onclick=()=>{
    const b=document.querySelector("#g-home");
    if(b && !S.settings.graphHome) b.click();
    turDone();
  };
  const цель=шаг.цель ? document.querySelector(шаг.цель) : null;
  if(цель) цель.classList.add("tour-lit");
  /* Позицию считаем СИНХРОННО, сразу после вставки разметки: размеры к этому моменту уже есть
     (чтение offsetWidth в turPlace само дожимает раскладку). Через requestAnimationFrame было
     нельзя — окно, которое не рисует кадров (свёрнуто, неактивно, за пределами экрана), rAF не
  ...
  /* ОТМЕЧАЕМ ПРИ ПОКАЗЕ, а не при закрытии. Закрытие ставило отметку только если тур ЗАКРЫЛИ
     ...
     подсказку, читать не стал и закрыл ОКНО. Отметка тогда не вставала, и при следующем
     запуске его встречало то же самое — то есть ровно то, чего быть не должно.
     Показали — значит человек её видел, второй раз не показываем. */
  if(S.settings.tourSeen!==ТУР_ID){ S.settings.tourSeen=ТУР_ID; persist(); }
}
/* ПОСЛЕ ОБНОВЛЕНИЯ ЧЕЛОВЕКА ВСТРЕЧАЕТ ПРЕДЛОЖЕНИЕ, А НЕ САМА ДЕМОНСТРАЦИЯ. Порядок такой:
   вышла версия → плашка «обновиться» → он жмёт, приложение перезапускается → и вот тут его
   ждёт короткая плашка «обновился, вот что нового, посмотреть?». Демонстрация со сравнением
   и стрелкой открывается ТОЛЬКО по кнопке.
   Сразу открывать демонстрацию нельзя: человек запустил приложение, чтобы работать, а ему
   поперёк экрана большое окно, которого он не просил. Плашка в углу спрашивает и не мешает.
   Отметку ставим НА ПОКАЗЕ плашки: увидел предложение — значит своё дело оно сделало, второй
   раз не предлагаем, даже если он закрыл окно не читая. */
function turBanner(){
  const было=$("#tour-banner"); if(было) было.remove();
  const b=el("div","tour-banner"); b.id="tour-banner";
  /* Подпись берём у ПЕРВОГО непоказанного шага, а не пишем руками: иначе каждый релиз про неё
     забывали бы, и плашка рассказывала бы про позапрошлую версию. */
  const нов=turFreezeNew();   // запомнили ДО того, как показ плашки проставит отметку
  const подпись = нов.length>1 ? "Несколько новых возможностей — показать по очереди?"
                : (нов[0] ? нов[0].ttl + "." : "Посмотреть, что изменилось?");
  b.innerHTML=`<i class="ti ti-bell tb-ico"></i>
    <div class="tb-txt"><b>Мыслик обновился — ${esc(ТУР_ID)}</b>
      <span class="tb-sub">${esc(подпись)}</span></div>
  ...
  setTimeout(()=>{ const n=$("#tour-banner"); if(n) n.classList.add("show"); }, 0);
  /* НЕ «esc»: так зовётся глобальное экранирование строк (core.js), и локальная функция с этим
     именем затеняет его во ВСЁМ теле turBanner — включая разметку выше, где ${esc(ТУР_ID)}
     тогда возвращает undefined. Поймано на живом показе: «Мыслик обновился — undefined». */
  function поEscape(e){ if(e.key==="Escape"){ e.preventDefault(); e.stopPropagation(); убрать(); } }
  const убрать=()=>{ const n=$("#tour-banner"); if(n) n.remove();
    document.removeEventListener("keydown", поEscape, true); };
  document.addEventListener("keydown", поEscape, true);
  $("#tb-close",b).onclick=убрать;
  $("#tb-go",b).onclick=()=>{ убрать(); turOpen(false); };   // после обновления — только новое
  if(S.settings.tourSeen!==ТУР_ID){ S.settings.tourSeen=ТУР_ID; persist(); }
}
function turMaybeShow(){
  // спрашиваем не «совпала ли отметка», а «есть ли что показать»: после заплатки новым может
  // оказаться один шаг из шести, и плашка обязана выйти именно ради него
  if(!turNewSteps().length) return false;
  // ждём, пока вид отрисуется: плашка садится поверх готового интерфейса
  setTimeout(()=>{ if(S.settings.tourSeen!==ТУР_ID) turBanner(); }, 700);
  return true;
}

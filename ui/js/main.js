...
function turPlace(){
  if(!_турУзел) return;
  const шаг=ТУР[_турШаг], цель=шаг.цель ? document.querySelector(шаг.цель) : null;
...
function turStep(n){
  _турШаг=n;
  const шаг=ТУР[n]; if(!шаг){ turDone(); return; }
  ...
  const последний = n===ТУР.length-1;
...
function turBanner(){
  ...
  $("#tb-go",b).onclick=()=>{ убрать(); turStep(0); };
  if(S.settings.tourSeen!==ТУР_ID){ S.settings.tourSeen=ТУР_ID; persist(); }
}
function turMaybeShow(){
  if(S.settings.tourSeen===ТУР_ID) return false;

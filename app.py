...
class Api:
    ...
    def ai_report(self, text, purpose="", mode="prose"):
        """Отчёт по выделенным заметкам/задачам (свободный текст).
        purpose — необязательная цель/адресат отчёта; mode — "prose" (связный текст)
        или "list" (список папок «владелец — путь», purpose работает как запрос-отбор)."""
        if not ai_mod:
            return {"ok": False, "error": "no_module"}
        try:
            return ai_mod.report(text or "", purpose or "", mode or "prose")
        except Exception as e:
    ...
    try:
        ...
        def _apply():
            try:
                ...
                # — границы берутся как есть. Увести рамку ЗА края экрана тоже не выходит: как
                # только прямоугольник вылезает за монитор, система снова прибавляет свои восемь.
...
def _hide_window_border(hwnd):
    ...
    try:
        ...
        # Границу просим НЕ рисовать вовсе, а не красить в цвет окна: развёрнутое окно теперь
        # лежит точно в границах экрана (иначе оно накрывало бы панель задач), и крашеная
        # граница стала видна серой полосой по бокам и снизу.
        res = attr(DWMWA_BORDER_COLOR, DWMWA_COLOR_NONE)
...
def _set_taskbar_button_icon(hwnd, icon_path):
    """Свойства окна для панели задач: иконка при закреплении + идентификатор группы.

    ЭТО ПОДСТРАХОВКА, А НЕ ГЛАВНОЕ РЕШЕНИЕ. Иконку кнопки определяет иконка ФОРМЫ, заданная
    до первого показа окна (см. webview.start(icon=...) в main). Свойство
    System.AppUserModel.RelaunchIconResource влияет на закреплённый ярлык и на перестроение
    группы — оно полезно, но само по себе не спасает: пока форма рождалась с иконкой
    python.exe, кнопка показывала питона, и «починка» этим свойством срабатывала лишь тогда,
    когда систему заставляли перечитать группу (например, открытием второго окна). Именно так
    первая попытка и выглядела успешной на снимке — рядом было открыто второе окно."""
...
def main():
    ...
    # sys.executable (winforms.py: ExtractIconW(..., sys.executable, 0)) и ставит её форме
    # в конструкторе — то есть ДО первого показа. Кнопка на панели задач создаётся вместе с
    # окном и запоминает именно эту иконку: при запуске из исходников — значок python.exe.
    # Наш WM_SETICON приходил на секунду позже и уже созданную кнопку не переубеждал; иконка
    # «появлялась» только при открытии второго окна, когда система перестраивала группу.

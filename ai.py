...
# Если заголовок заметно короче ввода (срезал >= стольких символов) — значит из
# мысли ушли детали, поэтому полный текст храним ДОСЛОВНО в описание (модель его НЕ
# переписывает — иначе теряется нужное). Порог по РАЗНИЦЕ, а не по длине: короткая
# правка с деталью («портал — убрать космос…») тоже сохранит полный текст.
_KEEP_DIFF = 15
...
_INSTRUCT = (
    ...
    "Заголовок — коротко, 2–5 слов, суть; ОБЯЗАТЕЛЬНО сохрани главный предмет "
    "(о чём это). Полный текст сохранится отдельно сам — впихивать всё в заголовок "
    "не надо.\n"
...
def capture(text):
    ...
    # Заголовок срезал заметный кусок → полный текст в описание ДОСЛОВНО (не
    # пересказ модели!). Ничего лишнего не режем; заголовок ≈ ввод → без описания.

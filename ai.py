...
_REPORT_INSTRUCT = (
    "Ты оформляешь отчёт из заметок и задач пользователя планировщика «Мыслик». В данных ниже "
    "у каждого пункта маркер: «•» — заметка, «✓» — выполненная задача, «►» — задача в работе, "
    "«○» — задача не начата. ОТСТУПЫ показывают иерархию: пункт с бо́льшим отступом подчинён "
    "тому, что над ним с меньшим отступом.\n"
    "ЖЁСТКИЕ ПРАВИЛА:\n"
    "1) Опирайся СТРОГО на данные. НЕ придумывай задачи, действия, статусы, выводы или детали, "
    "которых во вводе НЕТ. Ничего не додумывай.\n"
    "2) Сохраняй разметку: заметки остаются заметками, задачи — задачами с ТЕМ ЖЕ статусом. "
    "Если задач во вводе НЕТ — раздел задач НЕ создавай.\n"
    "3) СОХРАНЯЙ ИЕРАРХИЮ: вложенные (с отступом) пункты держи под их родителем, не разрывай "
    "связанные группы.\n"
    "4) Каждый пункт РОВНО один раз, без повторов. Сохраняй конкретику (что именно замечено/"
    "сломано/найдено), не выхолащивай.\n"
    "5) ГОЛОС: это отчёт САМОГО пользователя, а не пересказ со стороны. Пиши безлично/от первого "
    "лица о ФАКТАХ. НЕ пиши «пользователь сделал», «пользователь столкнулся», «пользователь "
    "нашёл» — пиши прямо: «на испытании с ловушками пропадает возможность бить», «нашёл, как "
    "использовать фласки».\n"
    "Оформи связно, деловым тоном, маркдаун-списками, без вступления и воды."
)

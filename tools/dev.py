...
# Потолок на один сценарий. Поднят с 60 с: «граф» вырос до двух сотен проверок, и часть из них
# ждёт, пока успокоится физика — на загруженной машине он упирался в старый лимит и падал
# «таймаутом», хотя отдельным прогоном проходил целиком. Ложный провал дороже лишних секунд.
SCEN_WAIT = 120

   ...
   такт синусу), а не дрожь физики. */
{
  ...
  if (н && дн.length === 10) {
    ...
    graph.drag = null; дрожь(false);
    /* Пересечения считаем ПОСЛЕ ОТПУСКАНИЯ и ещё двухсот кадров: пока ноду держат, расплетение
       нарочно почти молчит (иначе оно и даёт дрожь), а работу свою делает, когда рука отпустила.
       Мерить их во время драга значило бы требовать от силы того, от чего мы её сознательно
       освободили. */
    graph.alpha = 0.6;
    ...
    for (let i = 0; i < graph.links.length; i++) for (let j = i+1; j < graph.links.length; j++) {
      ...
      if ((пл(c,d,a)>0)!==(пл(c,d,b)>0) && (пл(a,b,c)>0)!==(пл(a,b,d)>0)) перекрестий++;
    }
    // Порог 8: как есть было 20–26 разворотов, с затуханием 1–4, без расплетения вовсе 0.
    ...
    t.push({имя:"после отпускания паутина всё равно расплетается", ок: перекрестий <= 9,
            факт: "перекрестий осталось " + перекрестий + " (без расплетения вовсе — 12-16)"});
  }

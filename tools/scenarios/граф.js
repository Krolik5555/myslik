   ...
   ноды по тому же правилу: у хаба своих детей в остове нет, ветку ему собирают по членству. */
{
  ...
  graph.drag=null;
  t.push({имя:"ветка идёт за нодой физикой, отставая",
          ок: догон>0.12 && догон<0.98,
          факт:"хвост прошёл "+Math.round(догон*100)+"% пути ноды"});
  ...
  if(хаб){
    ...
    const проба=вх[0].n, п0=проба.x, х0б=хаб.x;
    graph.drag=хаб;
    for(let i=0;i<40;i++){ хаб.x+=12; graph.alpha=Math.max(graph.alpha,.4); graph._tick(true); }
    const догонО=(проба.x-п0)/(хаб.x-х0б);
    graph.drag=null;
    t.push({имя:"ноды идут за областью, а не остаются на месте",
            ок: догонО>0.12, факт:"нода прошла "+Math.round(догонО*100)+"% пути области"});
  }

   ...
   ноды по тому же правилу: у хаба своих детей в остове нет, ветку ему собирают по членству. */
{
  ...
  const ветка=graph._branch(узел);
   ...
   для одного из двух рендеров. */
{
     ...
     до первого клика. */
  {
    ...
    t.push({имя:"холст: узел находится под курсором по координатам",
            ок: graph._hitNode({clientX:p.x, clientY:p.y}) === n.id,
            факт: "нашли " + (graph._hitNode({clientX:p.x, clientY:p.y}) || "никого")
    ...
    if (св >= 0){
      ...
      t.push({имя:"холст: связь находится по своей линии", ок: graph._hitLink({clientX:м.x, clientY:м.y}) >= 0,
              факт: "индекс " + graph._hitLink({clientX:м.x, clientY:м.y})});
    }
   ...
   ребро к хабу пропало, чужая область не тронута. */
{
  ...
  t.push({имя:"родня пустышки — только её СВОИ связи, без подмешивания связей хаба",
          ...
          факт: "родня пустышки = adj пустышки: " + (graph._kin(пуст.id) === graph.adj[пуст.id])
              + ", размер " + (graph.adj[пуст.id] ? graph.adj[пуст.id].size : "нет")});

  /* _fromLiveNode — ТЕПЕРЬ ПУТЬ ТОЛЬКО ДЛЯ СТАРЫХ ДАННЫХ: новые прикрепления (см. выше) уже не
     ...
     обратную совместимость, а не текущий путь создания. */
  {
    ...
    recomputeHierarchy(); graph.build();
    graph._fromLiveNode(пуст.id, нода.id);
     ...
     пустышке, а к ноде области»). */
  {
    ...
    const дочерняя = graph._createHollow(обл.id, базовая);
    recomputeHierarchy(); graph.build();
     ...
     собственные связи — расширение работает лишь в одну сторону, от хаба. */
  {
    ...
    recomputeHierarchy(); graph.build();
    const родняХаба = graph._kin("hub_" + обл.id);
    const родняПустышки = graph._kin(пуст3.id);
   ...
   миллисекундам был бы машинозависимым, а счётчик вызовов — нет. */
{
  ...
  try {
    ...
    await ж(220);                        // ...а через паузу — обязана (см. _syncAsideLater)
...
/* ГРУППОВОЕ ПЕРЕТАСКИВАНИЕ. Тащишь одну из выделенных — едут все выделенные, и форма группы
   ...
   _homeFromDrag такую ноду законно пропускает — проверять на ней назначение дома нечего. */
{
  // фикстура своя: детям нужен ВЛАДЕЛЕЦ (родитель или область), иначе дому не от чего считаться
  // и _homeFromDrag такую ноду законно пропустит — проверять на ней было бы нечего
  const было = S.items.length;
  ...
  /* Корень привязываем к ОБЛАСТИ: иерархию recomputeHierarchy считает обходом от областей, и
     ...
     назначается (см. _homeFromDrag), и проверка ниже мерила бы пустоту. */
